...
#define _DUTY_MAX 705 // 2000
#define _DUTY_NEU 1590 // 1500
#define _DUTY_MIN 2294 // 1000

#define _SERVO_ANGLE_DIFF  1589  // Replace with |D - E| degree
#define _SERVO_SPEED       3100  // servo speed
...
void setup()
{
  ...
  //  (float)(_DUTY_MAX - _DUTY_MIN) * ((float)_SERVO_SPEED / _SERVO_ANGLE_DIFF) * (_INTERVAL_SERVO / 1000.0); 
    (float)(_DUTY_MIN - _DUTY_MAX) * ((float)_SERVO_SPEED / _SERVO_ANGLE_DIFF) * (_INTERVAL_SERVO / 1000.0); 
}
...
void loop()
{
  ...
  if (event_dist) {
    ...
    float filtered = ir_sensor_filtered(10, 0.5, 1); // Replace n with your desired value
    float dist_raw = ((6762.0 / (filtered - 9.0)) - 4.0) * 10.0 -60;
    // get a distance reading from the distance sensor
    dist_filtered = precise_dist(dist_raw);
...
float precise_dist(unsigned int dist) 
{
  // Replace below line with the equation obtained from nonlinear regression analysis
  return -8.88 + 1.23*dist + 1.42E-03*pow(dist,2) + -4.25E-05*pow(dist,3) + 2.52E-07*pow(dist,4); //-8.88 + 1.23x + 1.42E-03x^2 + -4.25E-05x^3 + 2.52E-07x^4
}
